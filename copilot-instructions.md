...
- If in two tries, the bowler fails to knock down all the pins, their score for that frame is the total number of pins knocked down in their two tries.
...
What I would like from you is to pass the tests in `test_bowling.py` and writing the code in `bowling.py`. I will be adding the tests myself in `test_bowling.py`, so all I want you to do is to pass the tests that I write. The code should should be human readable adhering to encapsulation and clean code practices.

When you detect a change in `test_bowling.py`, update `bowling.py` using the red/green/refactor approach, writing the minimum code needed to pass the new test.
